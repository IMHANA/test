...
<%-- 전달받은 제품번호를 사용해서 DB데이터 조회 후 화면 표시 --%>
	...
	<%-- 액션태그 useBean : 현재 지정한 scope(session) 상에 id명(dao)의
	속성값이 있으면 가져다가 dao라는 로컬변수에 담아서 사용,
	scope상에 없으면 클래스속성 타입의 객체를 생성하고 scope를 새로 등록. --%>
...
<%
	...
	pageContext.setAttribute("svo", vo);
%>
	...
	<table>
		<tbody>
			...
			<tr>
				<th>제픔가격</th>
				<td>${svo.p_price}
					<span class="red">(할인가격 : ${svo.p_saleprice})</span></td>
			</tr>
			<tr>
				<th>제품설명</th>
				<td>${svo.p_content}</td>
			...
			<tr>
				<td colspan="2">
				<img src="images/${svo.p_image_l}" alt="제품이미지">
				</td>
			</tr>
		</tbody>
		<tfoot>
...
<script>
	function add() {
		location.href = "addProduct.jsp?p_num=${svo.p_num}";
	}
	function cart_go() {
