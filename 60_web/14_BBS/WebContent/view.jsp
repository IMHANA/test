...
<%-- 전달받은 파라미터 값 사용해서 DB데이터 조회 후 화면에 표시
	1. 게시글 조회수 1증가
	2. 게시글 (b_idx) 데이터 조회 후 화면에 표시
	3. 게시글 (b_idx)에 달린 댓글이 있으면 화면에 표시	
--%>
<%
	// 파라미터 값 확인
	...
	//1. 게시글 조회수 1증가 (실습)
	//BBS_T TABLE에 대한 update임 (CRUD - U)
	int board_idx = DAO.updateHit(Integer.parseInt(b_idx));
	System.out.println(">view board_idx : " + board_idx);
	
	//2. 게시글(b_idx) 데이터 조회
	BBSVO bvo = DAO.selectOne(b_idx);
	System.out.println(">view bvo : " + bvo);
	
	//3. 게시글 (b_idx)에 달린 댓글이 있으면 조회 (검색, 찾기)
...
<script>
	function modify_go() {	//수정페이지
		document.frm.action = "modify.jsp";
		document.frm.submit();
	}
	function delete_go() {	//삭제페이지
		document.frm.action = "delete.jsp";
		document.frm.submit();
	}
	function list_go() {	//list페이지
...
<form action="ans_write_ok.jsp" method="get">
	...
	<input type="submit" value="댓글저장 (실습)">
