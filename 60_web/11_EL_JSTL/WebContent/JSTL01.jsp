...
%>
	...
	<p>application test(EL) : 
		<c:out value=" ${applicationScope.test }" default="데이터없음"/></p>
	<hr>
	...
	<c:if test="true">
		<p>if의 test 속성값이 true인 경우 실행문장 (무조건실행됨)</p>
	</c:if>
	<c:if test="false">
		<p>if의 test 속성값이 false인 경우 실행이 안되는 문장</p>
	</c:if>
	
	<%-- JSTL에는 else문이 없다 (제공되지 않는다) 
		그래서 직접 else문을 할 if문을 작성해서 처리해야한다--%>
	<c:if test="${20 > 10 }">
		<p>20 &gt; 10 결과 true인 경우 실행문장</p>
	</c:if>
	<c:if test="${20 <= 10 }">	<%-- else역할 문장 --%>
	...
	<p>:: n1 > n2 result : ${result }</p>
	<c:if test="${n1 <= n2 }" var="result">
		<p>if문 결과 n1(${n1 })이 n2(${n2 })보다 작거나 같습니다 (result : ${result })</p>
	</c:if>
	<p>:: n1 <= n2 result : ${result }</p>
	<hr><hr>
...
%>	
	...
	arr[0] : ${attr_arr[0] } ::스코프(scope) 상에 등록되지 않으면 EL에서 사용못함
